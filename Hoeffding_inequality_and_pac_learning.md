...
```
As we know, $\nu$ is the model's error on the training data, sometimes it written as $E_{in}$. In addition, $\mu$ is the model's true error on new data, usually it written as $E_{out}$. In this case, we take hypothesis set size $M=1$. So, we get
```math
